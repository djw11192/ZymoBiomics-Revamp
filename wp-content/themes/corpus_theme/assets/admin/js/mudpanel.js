...
*/
    ...
    // Icon Switcher Plugin
    (function(){
        $.fn.mudPanelIconSwitcher = function (options) {

            if (this) {
                var thisParent = this;

                // Default Selection
                var strIconSelected = $(thisParent).find('input[type=radio]:checked').val();
                $(thisParent).find('i.mdf-' + strIconSelected).addClass('iconselected');

                // Manual Selection
                $(this).find('i').on('click', function () {
                    var strIconClicked = $(this).prev().val();
                    $(thisParent).find('i').removeClass('iconselected');
                    $(thisParent).find('i.mdf-' + strIconClicked).addClass('iconselected');
                });
            }
            return this;
        };
    })();
    $('.feature_icon_1').mudPanelIconSwitcher();
    $('.feature_icon_2').mudPanelIconSwitcher();
    $('.feature_icon_3').mudPanelIconSwitcher();
    $('.feature_icon_4').mudPanelIconSwitcher();
    ...
    (function(){
        $('#section-homepage_priority .mud-sortable-box ul').sortable({
            axis: 'y',
            stop: function (event, ui) {
                theArray = $('#section-homepage_priority .mud-sortable-box ul').sortable("toArray");
                $('input#homepage_priority').val(theArray.join(","));
            }
        });
    })();
});