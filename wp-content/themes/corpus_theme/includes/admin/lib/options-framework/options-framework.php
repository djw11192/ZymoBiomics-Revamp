 ...
 * Version: 1.3
 * 
 * @package mudThemes Options Panel
...
function mudthemes_optionsframework_setdefaults() {
	...
	// If the options haven't been added to the database yet, they are added now
        /* Use Sane Defaults : Modified by mudThemes
...
if ( !function_exists( 'mudthemes_optionsframework_add_page' ) ) {
	...
	function mudthemes_optionsframework_add_page() {
		$of_page = add_theme_page(__('Corpus Theme - Settings', 'corpus'), __('Theme Options', 'corpus'), 'edit_theme_options', 'corpus-options','mudthemes_optionsframework_page');
...
function mudthemes_optionsframework_load_styles() {
    wp_enqueue_style('open-sans', 'http://fonts.googleapis.com/css?family=Open+Sans:300,400,600,300italic,400italic,600italic');
    wp_enqueue_style('mudthemes-font-awesome', CORPUS_ASSETS_URL .'admin/css/font-awesome.'. corpus_get_scripts_info('font_awesome', 'version') .'.css', array(), corpus_get_scripts_info('font_awesome', 'version'));
    wp_enqueue_style('mudthemes-colorpicker', CORPUS_ASSETS_URL .'admin/css/colorpicker.css', array(), corpus_get_scripts_info('colorpicker', 'version'));
    wp_enqueue_style('mudthemes-panel', CORPUS_ASSETS_URL .'admin/css/mudthemes-panel.css', array(), corpus_get_scripts_info('mudpanel', 'version'));
}	
...
function mudthemes_optionsframework_load_scripts($hook) {
	...
	if ( 'appearance_page_corpus-options' != $hook )
        return;
	
	// Enqueued scripts
	wp_enqueue_script('jquery-ui-core');
	wp_enqueue_script('color-picker', CORPUS_ASSETS_URL .'admin/js/colorpicker.js', array('jquery'), corpus_get_scripts_info('colorpicker', 'version'));
	wp_enqueue_script('mudpanel', CORPUS_ASSETS_URL .'admin/js/mudpanel.js', array('jquery', 'jquery-color', 'jquery-ui-core', 'jquery-ui-sortable'), corpus_get_scripts_info('mudpanel', 'version'));
        wp_enqueue_script('smooth-scroll', CORPUS_ASSETS_URL .'admin/js/smooth-scroll.js', array(), corpus_get_scripts_info('smoothscroll', 'version'), TRUE);
...
function mudthemes_optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		add_settings_error( 'corpus-options', 'restore_defaults', __( '<i class="mdf mdf-undo"></i> Options reset', 'corpus' ), 'updated fade' );
	 ...
	 */
		...
		foreach ( $options as $option ) {
			...
			$id = preg_replace( '/[^a-zA-Z0-9._\-]/', '', strtolower( $option['id'] ) );
                        ...
                        if ( isset($input['skin']) && ($input['skin'] != corpus_get_option('skin')) ) {
                            $input = mudthemes_on_skin_change($input);
                        }
		...
		add_settings_error( 'corpus-options', 'save_options', __( '<i class="mdf mdf-check"></i> Options saved', 'corpus' ), 'updated fade' );
		return $clean;
	}
...
function mudthemes_optionsframework_adminbar() {
	...
	$wp_admin_bar->add_menu( array(
			...
			'title' => __( 'Theme Options', 'corpus' ),
			'href' => admin_url( 'themes.php?page=corpus-options' )
		));
}
...
/**
 * This functions adds options to Database when site is loaded for the first time.
 */
function mudthemes_add_options_to_db(){
    if ( $optionsfile = locate_template( array('options.php') ) ) {
        require_once($optionsfile);
    }
    ...
    if ( isset($values) && !get_option(corpus_get_theme_info('name'))) {
        add_option( corpus_get_theme_info('name'), $values ); // Add option with default settings
    }
}
add_action('init', 'mudthemes_add_options_to_db');
...
?>                      <div class="mud-header-menu">
                            <a href="<?php echo CORPUS_DOCS_URL ?>" target="_blank" title="Documentation"><i class="mdf mdf-edit"></i><span>Documentation</span></a>
                            <a href="<?php echo CORPUS_SUPPORT_URL ?>" target="_blank" title="Support"><i class="mdf mdf-user"></i><span>Support</span></a>
                            <a href="<?php echo CORPUS_CONTACT_URL ?>" target="_blank" title="Contact"><i class="mdf mdf-envelope"></i><span>Contact us</span></a>
                        </div>
                        <div class="mud-admin-logo">
                            <span class="mud-admin-logo-text-section"><i class="mdf mdf-check-square-o"></i><span class="mud-admin-logo-text">Corpus</span></span>
                            <span class="mud-admin-version">by <a href="http://www.mudthemes.com/" target="_blank" title="mudThemes | Premium WordPress Themes">mudThemes</a> | <?php echo 'version ' . corpus_get_theme_info('version') ?></span>
                        </div>
    			<div class="upgrade-box"><p><?php if(!CORPUS_PRO): _e('<i class="mdf mdf-quote-left"></i> Upgrade to Corpus Premium <i class="mdf mdf-quote-right"></i> and get Colorful skins with 500+ Google Fonts, Beautiful Homepage, Multiple slides and much more.', 'corpus'); ?> <a href="<?php echo CORPUS_PRO_URL ?>" target="_blank" title="Download Corpus Premium"><span>Download Corpus Premium</span></a> <span class="span-header-download"> <i class="mdf mdf-check"></i></span></p><?php else: _e(sprintf('<b><u>Notice:</u></b> A new Corpus Premium theme update will be released before <em>'.CORPUS_UPDATE_DATE.'</em>. Please see if any new update is available: <a href="%s" target="_blank">Check Here</a>', CORPUS_CHECK_VERSION_URL ), 'corpus'); endif ?></div>
...
function of_mud_scroll_top(){
    		...
    		$status =  get_settings_errors('corpus-options');
                if(!empty($status)):
...
function of_mud_submit_botton_bottom(){
    ...
    $jstext = esc_js(__('Click OK to reset. Any theme settings will be lost!', 'corpus'));
